...
static bool cvsw_do_match(const struct sk_buff *skb, const __u16 in_port, const struct cvsw_match *match)
{
	...
	} else if (tp->dest == htons(GENEVE_PORT)) {
	    struct genevehdr *geneve;
	    geneve = (struct genevehdr*)hdr;
	    if (~wildcards & OFPFW_EXT_TUN_GENEVE_VNI) {
		if (match->tun_id != geneve->vni) {
		    return false;
		}
		wildcards |= OFPFW_EXT_TUN_GENEVE_VNI;
	    }
	}
    ...
    } else if (nw->protocol == IPPROTO_GRE) {
	struct nvgrehdr *nvgre;
	nvgre = (struct nvgrehdr*)hdr;
	if (~wildcards & OFPFW_EXT_TUN_NVGRE_VSID) {
	    if (match->tun_id != nvgre->vsid) {
		return false;
	    }
...
static bool cvsw_apply_instruction(struct sk_buff *skb, const struct cvsw_instruction *inst)
{
    ...
    case CVSW_INST_TYPE_SET_NVGRE:
	cvsw_apply_set_nvgre(skb, &inst->tun_nvgre);
	break;
    case CVSW_INST_TYPE_STRIP_NVGRE:
	return cvsw_apply_strip_nvgre(skb);
    ...
    case CVSW_INST_TYPE_SET_GENEVE:
	cvsw_apply_set_geneve(skb, &inst->tun_geneve);
	break;
